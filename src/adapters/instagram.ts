  ...
  for (const blob of extractEmbeddedJsonBlobs(html)) {
    ...
    visitObjects(blob, (node: any) => {
      ...
      const followers =
        ...
        typeof node?.edge_followed_by?.count === 'number'
...
export const parseInstagram = (html: string, keyword: string): UnifiedLead[] => {
  ...
  // Regex fallback with optional follower count

  const leads: UnifiedLead[] = [];

  const regex =

    /"username":"([\w.]{2,30})".*?"full_name":"([^"]{0,120})".*?"external_url":"([^"]*)"(?:[\s\S]*?"edge_followed_by"\s*:\s*\{"count":(\d+))?/g;
  ...
  for (const m of html.matchAll(regex)) {
    ...
    const followers = m[4] ? Number(m[4]) : undefined;
